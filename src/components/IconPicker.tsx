...
import {
  ...
  Plus, Minus, Circle, Square, Triangle, Star,
    ...
    <>
      ...
      {showPicker && (
        <div className="border rounded-lg p-3 bg-white shadow-lg mt-2">
          ...
          >
            {MEDICAL_ICONS[activeCategory as keyof typeof MEDICAL_ICONS].map((item) => {
              ...
              return (
                <button
                  ...
                  title={item.label}
                >
                  <IconComponent className={`w-5 h-5 mb-1 ${value === item.name ? 'text-blue-600' : 'text-gray-700'}`} />
                  <span className={`text-[10px] font-medium text-center leading-tight ${value === item.name ? 'text-blue-600' : 'text-gray-600'}`}>
                    {item.label}
                  </span>
                </button>
              );
